...
def create_inbound_receipt(
    ...
    receipt_code = _generate_receipt_code(now, idempotency_key)

    # Idempotency check: if receipt with identical receipt_code exists, return it
    existing = db.execute(
    ...
    for item in sorted_items:
        ...
        inv_tx = InventoryTransaction(
            ...
            notes=f"Nhập kho thành phẩm: {payload.batch_name}",
            created_at=now,
        )
        db.add(inv_tx)
