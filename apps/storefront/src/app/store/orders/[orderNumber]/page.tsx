...
import { getStoreOrderDetail, type CommerceOrderDetail } from "@/lib/commerce";
...
export default function StoreOrderDetailPage() {
  ...
  useEffect(() => {
    ...
    getStoreOrderDetail(orderNumber).then(setOrder).catch((requestError) => setError(requestError instanceof ApiError ? requestError.message : "Không tải được đơn hàng"));
  }, [orderNumber]);
  ...
  return (
    <section>
      <Link className="button-ghost -ml-3" href="/store/orders"><Icon className="rotate-180" name="arrow-right" size={17} />Danh sách đơn</Link>
