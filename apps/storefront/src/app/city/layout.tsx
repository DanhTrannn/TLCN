...
export default function CityLayout({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    if (!loading && !customer) {
      router.replace("/login?returnTo=/city");
    }
  ...
  if (customer.role !== "city_planner" && customer.role !== "admin") {
    return (
      <main className="mx-auto max-w-3xl px-5 py-14 sm:px-6">
        <section className="surface-card p-8 text-center">
          <h1 className="admin-heading">Không có quyền truy cập</h1>
          <p className="mt-3 text-muted">Khu vực này chỉ dành cho quản lý thành phố.</p>
        </section>
      </main>
    );
  }
      ...
      <div className="grid items-start gap-6 lg:grid-cols-[15rem_minmax(0,1fr)]">
        <aside className="sticky top-16 z-30 -mx-1 rounded-2xl border border-line bg-surface/95 shadow-admin backdrop-blur lg:top-24 lg:mx-0">
          <div className="hidden border-b border-line px-5 py-4 lg:block">
            <p className="text-xs font-semibold uppercase tracking-[0.16em] text-muted">Quản lý thành phố</p>
          </div>
          <CityNav />
        </aside>
