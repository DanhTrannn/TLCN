...
- `extract_tables_to_landing` dùng `SparkSubmitOperator(application=..., application_args=[...])` với các arg template `{{ ti.xcom_pull(...) }}`. **Không truyền kwarg `master`** — provider `apache-spark 4.11.3` không có kwarg này; master lấy từ connection `spark_default` (host `spark://spark-master:7077`). Connection là DB-backed và được provision idempotently bởi service `airflow-init` (docker-compose.yml: `airflow connections delete spark_default || true && airflow connections add spark_default --conn-host spark://spark-master:7077 --conn-type spark` — Airflow 2.10.5 không có flag `-f`, delete-then-add cho overwrite semantics), không cần thao tác tay khi khởi tạo môi trường.
