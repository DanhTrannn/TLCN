...
class LogExportTest(unittest.TestCase):
    ...
    def test_export_is_deterministic_and_manifests_match(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            self.assertEqual(first_summary.files, first_summary.manifests)
            ...
            for manifest_path in first.rglob("*.manifest.json"):
                manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
                data_path = first / manifest["object_path"]
                compressed = data_path.read_bytes()
                self.assertEqual(manifest["sha256"], hashlib.sha256(compressed).hexdigest())
                self.assertEqual(manifest["compressed_bytes"], len(compressed))
                with gzip.open(data_path, "rt", encoding="utf-8") as stream:
                    self.assertEqual(manifest["line_count"], sum(1 for _ in stream))
